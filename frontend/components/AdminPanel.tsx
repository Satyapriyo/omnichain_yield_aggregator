...
export function AdminPanel({ strategies, onRefresh }: AdminPanelProps) {
    ...
    const handleToggleStrategy = async (strategyPubkey: string, currentStatus: boolean) => {
        setIsToggling(true)
        try {
            // Simulate API call to toggle strategy
            await new Promise(resolve => setTimeout(resolve, 1000))
            console.log(`Toggling strategy ${strategyPubkey} from ${currentStatus} to ${!currentStatus}`)
            onRefresh()
        } catch (error) {
    ...
    const handleEmergencyPause = async () => {
        if (!confirm('Are you sure you want to pause all strategies? This will stop new deposits.')) {
            return
        }
                            ...
                            >
                                <div className="flex-1">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${strategy.isActive
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                        APY: {strategy.apy}% | TVL: ${strategy.tvl.toLocaleString()} | Risk: {strategy.riskLevel}
                                    </div>
                                </div>
                                <button
                                    onClick={() => handleToggleStrategy(strategy.publicKey.toString(), strategy.isActive)}
                                    disabled={isToggling}
                                    className={`px-4 py-2 rounded-md text-sm font-medium ${strategy.isActive
                                            ? 'bg-red-600 text-white hover:bg-red-700'
                                            : 'bg-green-600 text-white hover:bg-green-700'
                                        } disabled:opacity-50 disabled:cursor-not-allowed`}
                                >
                                    {isToggling ? 'Processing...' : (strategy.isActive ? 'Pause' : 'Activate')}
                                </button>
                            </div>
                        ))}
                    </div>
